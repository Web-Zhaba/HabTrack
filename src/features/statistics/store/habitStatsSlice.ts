...
import type { HabitLogsState } from './habitLogsSlice';
import { getTodayKey } from '../../habits/lib/utils';

// Тип для поддержки обоих путей: state.habits (старый) и state.nodes (новый)
type StateWithHabitsOrNodes = {
  habits?: { items: Habit[] };
  nodes?: { items: Habit[] };
};

const selectHabitsOrNodes = (state: StateWithHabitsOrNodes) =>
...
export const selectCompletedDays = createSelector(
  [
    (state: { habitLogs: HabitLogsState } & StateWithHabitsOrNodes) => state.habitLogs.items,
    (state: { habitLogs: HabitLogsState } & StateWithHabitsOrNodes) => selectHabitsOrNodes(state),
  ],
  (logs, habits) => {
...
export const selectLogsByDate = (dateKey: string) =>
  createSelector([(state: { habitLogs: HabitLogsState }) => state.habitLogs.items], (logs) =>
    logs.filter((log) => log.date === dateKey),
  );

/**
 * Проверяет, выполнены ли все привычки сегодня
 */
export const selectAllHabitsCompletedToday = createSelector(
  [
    (state: { habitLogs: HabitLogsState } & StateWithHabitsOrNodes) => selectHabitsOrNodes(state),
...
/**
 * Проверяет, выполнена ли конкретная привычка сегодня
 */
export const selectHabitCompletedToday = (habitId: string) =>
  createSelector(
    [
      (state: { habitLogs: HabitLogsState; habits: { items: Habit[] } }) => state.habits.items,
      (state: { habitLogs: HabitLogsState; habits: { items: Habit[] } }) => state.habitLogs.items,
